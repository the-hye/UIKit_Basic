...
//  JRNL
//
//  Created by JIHYE SEOK on 5/10/24.
//
...
class JournalEntryDetailViewController: UITableViewController {
    ...
    private func getMapSnapshot() {
        if let lat = selectedJournalEntry?.latitude, let lon = selectedJournalEntry?.longitude {
            let options = MKMapSnapshotter.Options()
            options.region = MKCoordinateRegion(center: CLLocationCoordinate2D(latitude: lat, longitude: lon), span: MKCoordinateSpan(latitudeDelta: 0.01, longitudeDelta: 0.01))
